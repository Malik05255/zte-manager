...
object Mc801aProfile : RouterProfile {
    ...
    override val statusFields: Set<String> = linkedSetOf(
        ...
        // Connected-client surfaces. Several ZTE firmwares expose JSON arrays here. ZteHttpTransport
        // retries station/lan lists individually when a firmware omits them from a multi-data reply.
        "wifi_access_sta_num", "station_list", "lan_station_list",

        // Read-only access-control evidence observed across ZTE MC-series web UIs. These fields are
        // intentionally telemetry only: no blacklist write is enabled until the active firmware's
        // ACL_mode semantics and read-back behaviour are proven.
        "ACL_mode", "wifi_mac_black_list", "wifi_mac_white_list",
        "wifi_hostname_black_list", "wifi_hostname_white_list", "user_ip_addr",
        ...
        // Both structured and legacy neighbor surfaces are probed read-only.
        "current_cell_info", "neighbor_cell_info", "locked_cell_info", "ngbr_cell_info",
